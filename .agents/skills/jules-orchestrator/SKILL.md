...
This skill guides the agent in orchestrating development tasks with **Google Jules**, utilizing microtask decomposition, parallel dispatch, contract-first design, and clean Git pull/merge workflows.
...
3. **High Context Purity (コンテキスト純度)**:
   - 各Julesセッションには、そのタスクに関連する2〜4ファイルと単体テストのみに集中させることで、コード消失やデバッグ迷走を防ぐ。
4. **Clean Integration & Never Manual File-by-File Copying (パッチの安全な一括取り込み)**:
...
### Step 2: 共通インターフェースの先行コミット
分割前に、リポジトリに以下を作成・プッシュする：
